    ...
    var GenerateWordFile = function (params, done) {
        ...
        setParameters = function (params) {
            ...
            if (params.FileName == null || params.FileName == '') {
                if ($jq(document).find("title").text() != "")
                    parameters.FileName = $jq(document).find("title").text();
                else {
                    parameters.FileName = window.location.pathname.split("/").pop();
                    if (parameters.FileName.indexOf('.') > -1)
                        parameters.FileName = parameters.FileName.split(".")[0];
                }
                Title = parameters.FileName;
                parameters.FileName += '_' + (new Date().getMonth() + 1) + '-' + (new Date().getDate()) + '-' + (new Date().getFullYear());
            }
            else {
            ...
            parameters.FileName = parameters.FileName.replace(/[\|&;\$%@"<>\(\)\+,]/g, "");
            parameters.Title = params.Title == null ? Title : params.Title;
            ...
            if (params.HeaderFooters == null)
                ...
                if (params.HeaderFooters.length > 0) {
                    for (var i = 0; i < params.HeaderFooters.length; i++) {
                        parameters.HeaderFooters.push({
                            Header: params.HeaderFooters[i].Header != null ? params.HeaderFooters[i].Header : '',
                            Footer: params.HeaderFooters[i].Footer != null ? params.HeaderFooters[i].Footer : ''
                        });
                    }
                }
                else
        ...
        generateTemplateHeadFooter = function (DocParams) {
            ...
            if (DocParams.HeaderFooters.length == 0) {
                templates.push('<html xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns:m="http://schemas.microsoft.com/office/2004/12/omml"= xmlns="http://www.w3.org/TR/REC-html40">' +
                    '<body>' + FootNoteTemplateSeparator + '<div style="mso-element:header;" id="h1">' +
                    '<p class=MsoHeader></p></div><div style=\'mso-element:footer\' id=f1>' +
                    '<p class=MsoFooter></p></div></body></html>');
            }
            else {
                for (var i = 0; i < DocParams.HeaderFooters.length; i++) {
                    templates.push('<html xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns:m="http://schemas.microsoft.com/office/2004/12/omml"= xmlns="http://www.w3.org/TR/REC-html40">' +
                        '<body>' + FootNoteTemplateSeparator + '<div style="mso-element:header;" id="h' + (i + 1) + '">' +
                        '<p class=MsoHeader>' + (DocParams.HeaderFooters[i].Header != null ? DocParams.HeaderFooters[i].Header : '') + '</p>' +
                        '</div><div style=\'mso-element:footer\' id=f' + (i + 1) + '>' +
        ...
        addFootNotes = function (Body, FootNotesArray) {
            if (FootNotesArray.length > 0) {
                var numFootnotes = ($jq(Body).text().match(/{{FootNote}}/g) || []).length;
                var BodyReturn = Body;
                var FootNotesString = '<div style=\'mso-element:footnote-list\'><![if !supportFootnotes]><br clear=all><hr align=left size=1 width="33%"><![endif]>';
                for (var i = 0; i < numFootnotes; i++) {
